...
module.exports = {
  ...
  renderClass: function(className, options) {
    ...
    while (classes.length !== 0) {
      this.renderElement(classes[0], options);
    }
  },
};
 ...
 *                           <statement> )[0..n]
 *
 *     <control>       :== <if> | <for> | <while> | <repeat> | <upon> | <match>
 *     <match>         :== \MATCH{<cond>} ( \CASE{<cond>} <block> \ENDCASE )[0..n]
 *                                        ( \OTHERWISE <block> )[0..1]
 *                                        \ENDMATCH
 *     <if>            :== \IF{<cond>} <block>
 *                         ( \ELIF{<cond>} <block> )[0..n]
...
Parser.prototype._parseControl = function() {
  ...
  if ((controlNode = this._parseMatch())) return controlNode;
};
...
Parser.prototype._parseMatch = function() {
  if (!this._lexer.accept('func', 'match')) return null;

  var matchNode = new ParseNode('match');

  // { <cond> } <match_cases>
  this._lexer.expect('open');
  matchNode.addChild(this._parseCond());
  this._lexer.expect('close');

  // ( \CASE { <cond> } <block> \ENDCASE )[0...n]
  var numCases = 0;
  while (this._lexer.accept('func', 'case')) {
    this._lexer.expect('open');
    matchNode.addChild(this._parseCond());
    this._lexer.expect('close');
    matchNode.addChild(this._parseBlock());
    this._lexer.expect('func', 'endcase');
    numCases++;
  }

  // ( \OTHERWISE <block> )[0..1]
  var hasOtherwise = false;
  if (this._lexer.accept('func', 'otherwise')) {
    hasOtherwise = true;
    matchNode.addChild(this._parseBlock());
  }

  // \ENDMATCH
  this._lexer.expect('func', 'endmatch');

  matchNode.value = {numCases: numCases, hasOtherwise: hasOtherwise};
  return matchNode;
};
...
Renderer.prototype._buildTree = function(node) {
  ...
  case 'match':
    ...
    // ==>
    // <p class="ps-line">
    //     <span class="ps-keyword">match</span>
    //     ...
    //     <span class="ps-keyword">end match</span>
    // </p>
    this._newLine();
    this._typeKeyword('match ');
    var matchCond = node.children[0];
    this._buildTree(matchCond);
    this._typeKeyword(' do');
    this._beginBlock();

    // ( \CASE {<cond>} <block> )[0..n]
    var numCases = node.value.numCases;
    for (var mi = 0 ; mi < numCases; mi++) {
      // \CASE {<cond>}
      // ==>
      // <p class="ps-line">
      //      <span class="ps-keyword">case</span>
      //      ...
      //      <span class="ps-keyword">end case</span>
      // </p>
      this._newLine();
      this._typeKeyword('case ');
      var caseCond = node.children[1 + 2 * mi];
      this._buildTree(caseCond);
      this._typeKeyword(' of');

      // <block>
      var caseBlock = node.children[1 + 2 * mi + 1];
      this._buildCommentsFromBlock(caseBlock);
      this._buildTree(caseBlock);

      if (!this._options.noEnd) {
        // ENDCASE
        this._newLine();
        this._typeKeyword('end case');
      }
    }

    // ( \OTHERWISE <block> )[0..1]
    var hasOtherwise = node.value.hasOtherwise;
    if (hasOtherwise) {
      // \OTHERWISE
      // ==>
      // <p class="ps-line">
      //     <span class="ps-keyword">otherwise</span>
      // </p>
      this._newLine();
      this._typeKeyword('otherwise');

      // <block>
      var otherwiseBlock = node.children[node.children.length - 1];
      this._buildCommentsFromBlock(otherwiseBlock);
      this._buildTree(otherwiseBlock);
    }
